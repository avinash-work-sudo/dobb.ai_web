...
export const GradientCard = ({ 
  ...
  // Use plan data if provided, otherwise use individual props
  const cardTitle = plan?.name || title;
  ...
  return (
    <div className={`${variant === 'pricing' ? 'w-full h-auto' : 'w-full h-screen flex items-center justify-center bg-black'}`}>
      ...
      >
        ...
        >
          ...
          >
            ...
            />
            {/* Icon - use provided icon or default star */}
 ...
 {variant === 'pricing' && cardMissingFeatures.length > 0 ? (
            ...
            {/* Button */}
            <motion.button
              onClick={plan?.buttonAction || buttonAction}
              className="inline-flex items-center justify-center w-full px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white text-sm font-medium rounded-lg transition-all duration-300 group"
              initial={{ filter: "blur(3px)", opacity: 0.7 }}
              animate={{
                filter: "blur(0px)",
                opacity: 0.9,
