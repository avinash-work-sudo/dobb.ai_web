...
//       try {
//         await this.renameMidsceneReport();
//       } catch (renamingError) {
//         console.warn('Midscene report renaming skipped:', renamingError.message);
//       }
//     }
...
//       /(?:goto|go to|visit)\s+([a-zA-Z0-9.-]+).*(?:and\s+)?(?:add|buy|purchase|find)\s+(?:a\s+)?([a-zA-Z0-9\s-]+?)\s+(?:to\s+)?(?:my\s+)?(?:cart|basket)/i;
...
//     const urlPattern =
//       /(?:^|First\s+|Then\s+)?(?:navigate to|go to|goto|visit|open)\s*([a-zA-Z0-9.-]+(?:\.[a-zA-Z]{2,})?(?:\/\S*)?)/i;
...
//         keycode: 'keycode.keyvalue.systems'
//       };
...
//       } catch (error) {
//         console.log(
//           `⚠️ Failed to navigate to ${url}, will try AI-based navigation: ${error.message}`
//         );
//         return task;
//       }
//     }
...
//   async renameMidsceneReport() {
//     if (!this.executionId) {
//       console.warn('No executionId available for midscene report renaming');
//       return;
//     }
...
//         // Find playwright report files and get the most recent one
...
//         if (playwrightFiles.length === 0) continue;

//         // Get stats for each file to find the most recent
//         const filesWithStats = await Promise.all(
//           playwrightFiles.map(async ({ file, filepath }) => {
//             try {
//               const stats = await stat(filepath);
...
//         // Create new filename with executionId
//         const newFilename = `playwright-report_${this.executionId}.html`;
//         const newFilepath = join(dir, newFilename);

//         // Rename the file
//         const fs = await import('fs/promises');
//         await fs.rename(mostRecent.filepath, newFilepath);

//         console.log(`✅ Renamed midscene report: ${mostRecent.file} → ${newFilename}`);
//         return newFilepath;
//       } catch (error) {
//         console.warn(`Failed to rename midscene report in ${dir}:`, error.message);
//       }
//     }
